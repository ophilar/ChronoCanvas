...
export const ArtworkCanvas: React.FC<ArtworkCanvasProps> = ({
  ...
  // Persistent WebGPU rendering effect
  useEffect(() => {
    ...
    img.src = currentDisplayLayer.imageUrl;

    img.onload = async () => {
      if (!active || !webGpuCanvasRef.current) return;
      try {
        const renderer = getWebGpuRenderer();
        await renderer.render(webGpuCanvasRef.current, img, webGpuOptions);
      } catch (err: any) {
  ...
  }, [enableWebGPU, currentDisplayLayer?.imageUrl, webGpuOptions]);
  ...
  return (
    <div className="relative w-full max-w-4xl h-full md:min-h-[60vh] md:max-h-[85vh] shadow-2xl rounded-sm overflow-hidden bg-white flex flex-col justify-center items-center">
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden bg-white flex items-center justify-center">
        {enableWebGPU ? (
          <canvas ref={webGpuCanvasRef} className="absolute inset-0 w-full h-full object-contain" />
        ) : layers.length > 0 ? (
